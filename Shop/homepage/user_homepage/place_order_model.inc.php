...
function set_order_placed_guest(object $pdo, string $user_address, float $total_price)
{
    $query = "INSERT INTO order_placed (user_address, total_price) VALUES (:user_address, :total_price);";
    $stmt = $pdo->prepare($query);
    $stmt->bindParam(":user_address", $user_address);
    $stmt->bindParam(":total_price", $total_price);
    $stmt->execute();

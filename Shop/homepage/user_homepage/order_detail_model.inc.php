...
function get_order_details_guest(object $pdo, int $guest_order_id) {
    $query = "SELECT * FROM order_placed WHERE order_id = :guest_order_id ORDER BY order_date DESC;";
    $stmt = $pdo->prepare($query);
    $stmt->bindParam(":guest_order_id", $guest_order_id, PDO::PARAM_INT);
    $stmt->execute();
